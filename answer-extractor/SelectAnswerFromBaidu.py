...
answerf = open('answerFromBaidu.txt', 'w')
counter = 0
...
for line in queryf:
    ...
    baiduf = open('openEvidence\\' + str(counter) + '.txt')

    for line in baiduf:
        oneLine = line.strip().replace('...', ' ')

        segmentedWords = pseg.cut(oneLine)
        evidence.append(list())

        for w in segmentedWords:
            if w.word != '':
                evidence[-1].append(w.word)

                if w.flag in questionType and w.word not in question:
                    candidateAnswers.append(w.word)
